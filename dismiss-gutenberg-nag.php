 ...
 *
 * Please Jeff Starr do not clone my code without declare it and give me appropriate rights. Thanks!
 *
 ...
 *
 * admin-footer-version-rebranded.php by Luciano Croce <luciano [dot] croce [at] googlemail [dot] com>
 * Copyright 2017-2018 [Luciano Croce](https://github.com/luciano-croce/) (email: <luciano [dot] croce [at] googlemail [dot] com>)
 *
 ...
 *
 * admin-footer-version-rebranded by by Luciano Croce <luciano [dot] croce [at] googlemail [dot] com>
 * Copyright 2017-2018 [Luciano Croce](https://github.com/luciano-croce/) (email: <luciano [dot] croce [at] googlemail [dot] com>)
 *
 * hide-try-gutenberg-callout.php by Paul V. Biron <paul [at] sparrowhawkcomputing [dot] com>
 * Copyright 2018 [Paul V. Biron](https://github.com/pbiron/) (email: <paul [at] sparrowhawkcomputing [dot] com>)
 ...
 */
	 ...
	 * History:
	 *
	 * 1.0.4 Expected 3rd future update in fourth quarter 2018
	 * 
	 ...
	 * @todo       Add option on settings control panel to completely remove Gutenberg when it is embedded on core of WordPress 5.0 - Make a standalone plugin settings page - Preemptive support for WordPress 5.0-alpha
	 */
...
/**
 * Defined requested variables to meet "Deailed Plugins Guidelines".
 *
 * @author  Luciano Croce <luciano.croce@gmail.com>
...
if ( version_compare( $version, '4.9-beta3', '<' ) ) {
	if ( version_compare( $version, '2.5', '<' ) ) {         # change 2.5 with latest version available it if you prefer die notification for all WordPress Releases
		if ( version_compare( $version, '2.3', '>=' ) ) {
			wp_die( __( 'This plugin requires WordPress 4.9-beta3 or greater:', 'dismiss-gutenberg-nag' ) . __( 'Activation Stopped!', 'dismiss-gutenberg-nag' ) . '<br /><br />' . __( 'Please note that "Gutenberg Dashboard Widget Nag" was introduced since WordPress 4.9-beta3.', 'dismiss-gutenberg-nag' ) );
		}
	}
	...
	function ddwtgn_ant_wp_version_init() {
		...
		<div class="notice notice-error is-dismissible error">
		<p>
		<?php _e( 'This plugin requires WordPress 4.9-beta3 or greater: please note that "Gutenberg Dashboard Widget Nag" was introduced since WordPress 4.9-beta3.', 'dismiss-gutenberg-nag' );?>
		</p>
		</div>
		<div class="notice notice-warning is-dismissible updated">
		<p>
		...
		include( ABSPATH . WPINC . '/version.php' );
		$version = str_replace( '-src', '', $wp_version );
		if ( version_compare( $version, '2.5', '>=' ) ) { _e( 'Plugin Dismiss Gutenberg Nag <strong>deactivated</strong>.', 'dismiss-gutenberg-nag' ); }
		?>
		<?php 
		global $wp_version;
		include( ABSPATH . WPINC . '/version.php' );
		...
		if ( version_compare( $version, '2.5', '<' ) ) {
		?>
		<?php _e( 'Plugin Dismiss Gutenberg Nag <strong>can not deactivate automatically</strong>. It is not compatible with this installation. Deactivate it manually!', 'dismiss-gutenberg-nag' );?> <!-- Thanks to Garrett Hyder (garrett-eclipse) for this contribute -->
		<?php 
		}
		}
		?>
...
//			add_filter( 'gutenberg_can_edit_post_type', array( $this, 'can_use_gutenberg' ) );    # This filter is provided by Gutenberg itself. Warning: disable Gutenberg demo !?
	 ...
	 *
	 * If the callout is dismissed for a user by our settings then that user will not be
 ...
 ?>
	...
	<script>
		...
		jQuery( 'document' ).ready( function( $ ) {
			var yes_always = $( '#dismiss_gutenberg_nag_options_always-yes' );
...
<?php
	...
	/**
	 * Render our "Dismiss for all users" setting.
	 *
	...
	function all_users_setting() {
		$option = get_option( 'dismiss_gutenberg_nag_options' );
 ?>
	<p id="dismiss_gutenberg_nag_options_always-options" style="margin: 0;">
		<input type="radio" name="dismiss_gutenberg_nag_options[all_users]" id="dismiss_gutenberg_nag_options_always-yes" value="yes"<?php checked( $option['all_users'], 'yes', true ) ?> />
		<label for="dismiss_gutenberg_nag_options_all_users-yes">
		...
		<input type="radio" name="dismiss_gutenberg_nag_options[all_users]" id="dismiss_gutenberg_nag_options_always-no" value="no"<?php checked( $option['all_users'], 'no', true ) ?> />
		<label for="dismiss_gutenberg_nag_options_all_users-yes">
...
<?php
	...
	<p id="dismiss_gutenberg_nag_options_specific_users-options" style="margin: 0;">
...
<?php
 ...
 <?php
	...
	/**
	 * Render our "Dismiss for all users without edit_posts" capability settings.
	 *
	 * Outputs "Yes" a/o "No" radio buttons.
	 *
	 * @return void
 ...
 ?>
	...
	<p id="dismiss_gutenberg_nag_options_for_non_edit_posts-options" style="margin: 0;">
		<input type="radio" name="dismiss_gutenberg_nag_options[non_edit_posts]" id="dismiss_gutenberg_nag_options_non_edit_posts-yes" value="yes"<?php checked( $option['non_edit_posts'], 'yes', true ) ?> />
		<label for="dismiss_gutenberg_nag_options_non_edit_posts-yes">
			<?php _e( 'Yes', 'dismiss-gutenberg-nag' ); ?>
			<span class='description'>
			<?php
					echo str_repeat( '&nbsp;', 5 );
		...
		<input type="radio" name="dismiss_gutenberg_nag_options[non_edit_posts]" id="dismiss_gutenberg_nag_options_non_edit_posts-no" value="no"<?php checked( $option['non_edit_posts'], 'no', true ) ?> />
		<label for="dismiss_gutenberg_nag_options_non_edit_posts-yes">
			<?php _e( 'No', 'dismiss-gutenberg-nag' ); ?>
			<span class='description'>
			<?php
...
<?php
	...
	/**
	 * Render our Dismiss Gutenberg Nag setting.
	 *
	 * Outputs Yes a/o No radio buttons.
	 *
	 * @return void
	...
	function disable_gutenberg_setting() {
		$option = get_option( 'dismiss_gutenberg_nag_options' );
 ?>
	<p id="dismiss_gutenberg_nag_options_disable_gutenberg-options" style="margin: 0;">
		<input type="radio" name="dismiss_gutenberg_nag_options[disable_gutenberg]" id="dismiss_gutenberg_nag_options_disable_gutenberg-yes" value="yes"<?php checked( $option['disable_gutenberg'], 'yes', true ) ?> />
		<label for="dismiss_gutenberg_nag_options_disable_gutenberg-yes">
			<?php _e( 'Yes', 'dismiss-gutenberg-nag' ); ?>
			<span class='description'>
				<?php
					echo str_repeat( '&nbsp;', 5 );
		...
		<input type="radio" name="dismiss_gutenberg_nag_options[disable_gutenberg]" id="dismiss_gutenberg_nag_options_disable_gutenberg-no" value="no"<?php checked( $option['disable_gutenberg'], 'no', true ) ?> />
		<label for="dismiss_gutenberg_nag_options_disable_gutenberg-no">
			<?php _e( 'No', 'dismiss-gutenberg-nag' ); ?>
			<span class='description'>
				<?php
					echo str_repeat( '&nbsp;', 5 );
